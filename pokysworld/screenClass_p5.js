export class GameScreen {
    ...
    shadeSky(p5, game) {
        const H = this.h/40;
        const num = this.h/H;
        const C1 = p5.color(game.levelData[game.currentLevel].skyStart);
        const C2 = p5.color(game.levelData[game.currentLevel].skyEnd);
        p5.noStroke();
        let rectColor;
        for (let i = 0; i < num; i++){
        ...
        game.levelData[game.currentLevel].levelEffects.forEach((effect, index) => {
            ...
            while(numB > 0){
                tempArrB.push(
                    new obj(
                        p5,
                        this.P.x + p5.floor(p5.random(this.w)),
                        this.P.y + p5.floor(p5.random(this.h)),
                        0.5,
                        1
                    )
                );
